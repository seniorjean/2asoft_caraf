...
<script>
	...
	$(document).ready(function () {
		...
		init_payment_process = function(day , month , year){
			payment_date = month+'/'+day+'/'+year;
			amount = montant_cotisation * 100;
			handler.open({
				name: 'CARAF',
		...
		function collect_data(stripe_token , stripe_response){
			...
			collected_data.card_holder      = '<?=$profile->first_name.' '.$profile->last_name;?>';
			collected_data.first_name		= '<?=$profile->first_name;?>';
			collected_data.last_name		= '<?=$profile->last_name;?>';
			collected_data.email			= '<?=$profile->email;?>';
			collected_data.advance_cotisation_month_count = 1;
			collected_data.advance_cotisation_month_string = payment_date;

			show_loader();

			$.post(base_url+'stripe_web/stripe_payment_process',collected_data,function () {
				hide_loader();
