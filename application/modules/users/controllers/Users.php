...
class Users extends MY_Controller {
    ...
    public function print_users()
     ...
     public function Profile()
     {
        ...
        $data['user_profile'] = $this->common_model->getUserProfile($user_id);

        $data['page'] = "users/users/user_profile";
        $this->template->template_view($data);  
     }

	public function member_profile($user_id = '')
	{
		if(!empty($user_id)){
			$data['user_profile'] = $this->common_model->getMemberProfile($user_id);
			$data['page'] = "users/users/member_profile";
	    ...
	    if(!empty($_POST)){
            if($action == 'toggle_user'){
                if(!empty($_POST['user_id'])){
                    $this->db->where(['id'=>$_POST['user_id']])->update('users',['active'=>$_POST['active']]);
                }
            }
        }
         ...
         if($action == 'delete'){
             if(!empty($_POST['user_id'])){
                 $this->db->where(['id'=>$_POST['user_id']])->delete('users');
             }
         }
     }
}
